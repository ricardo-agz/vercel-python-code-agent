...
export const ProjectsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const initialProjectId = React.useMemo(
    () => (persisted?.activeProjectId || `proj_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,7)}`),
    [persisted]
  );

  const [projects, setProjects] = React.useState<ProjectTab[]>(() => (
    persisted?.projects?.length ? persisted.projects : [{ id: initialProjectId, name: 'Project 1' }]
  ));
  ...
  const [projectStates, setProjectStates] = React.useState<Record<string, ProjectState>>(() => {
  ...
  const setActiveProjectId = React.useCallback((id: string) => {
    ensureProject(id);
  ...
  const isPathIgnored = React.useMemo(() => {
    ...
    const linesGit = gitignoreText ? gitignoreText.split(/\r?\n/).map(sanitize).filter(l => l && !l.startsWith('#')) : [];
    const linesAgent = agentignoreText ? agentignoreText.split(/\r?\n/).map(sanitize).filter(l => l && !l.startsWith('#')) : [];
  ...
  const projectForTree = React.useMemo(() => {
    ...
    for (const [p, c] of Object.entries(activeState.files)) {
      if (!isPathIgnored(p) || p === activeState.activeFile) out[p] = c as string;
    }
    return out;
  ...
  const projectForSend = React.useMemo(() => {
    const out: Record<string, string> = {};
    for (const [p, c] of Object.entries(activeState.files)) {
      if (!isPathIgnored(p) || p === '.gitignore' || p === '.agentignore') out[p] = c as string;
    }
    return out;
  ...
  const deleteFile = React.useCallback((filePath: string) => {
    ...
    if (activeState.activeFile === filePath) {
      const remaining = Object.keys(activeState.files).filter(p => p !== filePath).sort();
      setActiveFile(remaining[0] || '');
    }
  }, [setProject, setProposals, activeState.activeFile, activeState.files, setActiveFile]);

  const moveFile = React.useCallback((src: string, destDir: string) => {
  ...
  const createFolder = React.useCallback((folderPath: string) => {
  ...
  const deleteFolder = React.useCallback((folderPath: string) => {
    ...
    if (activeState.activeFile === normalized || activeState.activeFile.startsWith(normalized + '/')) {
      const remaining = Object.keys(activeState.files).filter(p => !(p === normalized || p.startsWith(normalized + '/'))).sort();
      setActiveFile(remaining[0] || '');
    }
  }, [setProject, setProposals, setFolders, activeState.activeFile, activeState.files, setActiveFile]);

  const renameFolder = React.useCallback((oldPath: string, newPath: string) => {
    ...
    if (activeState.activeFile === normalizedOld || activeState.activeFile.startsWith(normalizedOld + '/')) {
      const suffix = activeState.activeFile.slice(normalizedOld.length);
      setActiveFile((normalizedNew + suffix).replace(/^\//,''));
    }
  }, [setProject, setProposals, setFolders, activeState.activeFile, setActiveFile]);

  const moveFolder = React.useCallback((srcFolder: string, destDir: string) => {
    ...
    if (activeState.activeFile === normalizedSrc || activeState.activeFile.startsWith(normalizedSrc + '/')) {
  ...
  const upsertFileIfMissing = React.useCallback((filePath: string, content: string) => {
    if (isPathIgnored(filePath)) return;
    const normalized = (filePath || '').replace(/^\//,'');
    setProject(prev => (prev[normalized] !== undefined ? prev : { ...prev, [normalized]: (content ?? '') }));
  }, [setProject, isPathIgnored]);

  // Public project management operations
  const createProject = React.useCallback((name: string, templateId: string) => {
  ...
  const cloneProject = React.useCallback((id: string, newName?: string) => {
  ...
  }, [projects, activeProjectId, setProjects, setProjectStates, setActiveProjectId]);

  const setLoadingForProject = React.useCallback((projectId: string, v: boolean) => {
    setProjectStates(prev => {
