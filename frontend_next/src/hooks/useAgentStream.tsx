...
export function useAgentStream({ onMessage }: UseAgentStreamProps) {
  ...
  const connect = useCallback((runId: string, streamToken: string) => {
    const url = `${apiOrigin}/api/runs/${encodeURIComponent(runId)}/events?token=${encodeURIComponent(streamToken)}`;
  ...
  const resume = useCallback((runId: string, resumeToken: string, result: string) => {
    const url = `${apiOrigin}/api/runs/${encodeURIComponent(runId)}/resume?token=${encodeURIComponent(resumeToken)}&result=${encodeURIComponent(result)}`;
    const es = new EventSource(url);
    sourcesRef.current[runId] = es;
    es.onmessage = (evt) => {
      try {
