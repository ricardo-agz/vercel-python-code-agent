...
import { File as FileIcon, Image as ImageIcon, Archive, ChevronRight, ChevronDown } from 'lucide-react';
import { SiJavascript, SiTypescript, SiPython, SiReact, SiJson, SiMarkdown, SiHtml5, SiCss3, SiSass, SiGo, SiRust, SiC, SiCplusplus, SiRuby, SiPhp, SiGnubash, SiYaml } from 'react-icons/si';
...
type ContextMenuState = { x: number; y: number; path: string; type: 'file' | 'folder' | 'root' } | null;
...
function renderFileIcon(path: string): React.ReactNode {
  ...
  if (IMAGE_EXTS.includes(ext)) return <ImageIcon size={FILE_ICON_SIZE} color={FILE_ICON_COLOR} />;
  if (ARCHIVE_EXTS.includes(ext)) return <Archive size={FILE_ICON_SIZE} color={FILE_ICON_COLOR} />;
  return <FileIcon size={FILE_ICON_SIZE} color={FILE_ICON_COLOR} />;
}

const sortNodes = (a: TreeNode, b: TreeNode) => (a.type === b.type ? a.name.localeCompare(b.name) : a.type === 'folder' ? -1 : 1);
...
function buildTree(paths: string[], folders?: string[]): TreeNode {
  ...
  for (const p of paths.sort()) {
    ...
    for (let i = 0; i < parts.length; i += 1) {
      ...
      if (!cur.children) cur.children = [];
      if (isFile) {
        const fileNode: TreeNode = { name, path: parts.slice(0, i + 1).join('/'), type: 'file' };
        if (!cur.children.find(c => c.path === fileNode.path)) cur.children.push(fileNode);
      } else {
...
export const FileTree: React.FC<FileTreeProps> = ({ project, activeFile, onSelect, onCreateFile, onCreateFolder, onRename, onDelete, onMoveFile, onMoveFolder, proposed, folders, onRenameFolder, onDeleteFolder, expandedPaths, onExpandedChange }) => {
  ...
  const treeRoot = React.useMemo(() => {
    ...
    for (const k of keys) {
      const parts = k.split('/');
      for (let i = 1; i < parts.length; i += 1) folderList.push(parts.slice(0, i).join('/'));
    }
  ...
  const renamingKey = React.useMemo(() => renaming ? `${renaming.type}:${renaming.path}` : '', [renaming]);
  React.useEffect(() => {
  ...
  const commitCreate = React.useCallback(() => {
    ...
    if (!raw) { setCreating(null); return; }
    const fullPath = ((creating.parent ? creating.parent.replace(/\/$/, '') + '/' : '') + raw).replace(/^\//, '');
  ...
  const commitRename = React.useCallback(() => {
    ...
    if (!raw) { setRenaming(null); return; }
  ...
  const openContextMenu = React.useCallback((e: React.MouseEvent, path: string, type: 'file' | 'folder' | 'root') => {
    e.preventDefault();
    e.stopPropagation();
    setContextMenu({ x: e.clientX, y: e.clientY, path, type });
  }, []);

  const onDragOverTarget = React.useCallback((e: React.DragEvent, target: string | 'root') => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const onDropTo = React.useCallback((e: React.DragEvent, destDir: string) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const renderFileRow = (node: TreeNode, depth: number): React.ReactNode => {
    const isProposed = proposed && Object.prototype.hasOwnProperty.call(proposed, node.path);
      ...
      return (
        ...
        >
          <input
            ...
            style={{ color: '#dddddd', backgroundColor: '#1f1f22', border: '1px solid #3e3e42', padding: '2px 4px' }}
            value={renaming.name}
    ...
    return (
      <div
        ...
        title={node.path}
      >
        <span className="mr-2 flex items-center" aria-hidden="true">{renderFileIcon(node.path)}</span>
        <span className="truncate">{node.name}</span>
        {isProposed ? <span className="ml-2 text-xs" style={{ color: '#80e080' }}>(proposed)</span> : null}
      </div>
    );
  };
  ...
  const renderFolderNode = (node: TreeNode, depth: number): React.ReactNode => {
    ...
    return (
      <div key={node.path || '/'}>
        {node.path && (
          renaming && renaming.type === 'folder' && renaming.path === node.path ? (
            <div className="px-2 py-1" style={{ paddingLeft: 8 + depth * 12 }} onClick={(e) => e.stopPropagation()}>
              <input
                ref={inputRef}
                className="w-full text-sm"
                style={{ color: '#dddddd', backgroundColor: '#1f1f22', border: '1px solid #3e3e42', padding: '2px 4px' }}
            ...
            <div
              ...
              style={{
                ...
                paddingLeft: 8 + depth * 12,
                backgroundColor: dragOver === node.path ? 'var(--vscode-hover)' : 'transparent'
              }}
            ...
            >
              {isExpanded(node.path) ? (
                <ChevronDown size={12} color="#a0a0a0" className="mr-1" aria-hidden="true" />
              ) : (
                <ChevronRight size={12} color="#a0a0a0" className="mr-1" aria-hidden="true" />
              )}
              <span>{node.name}</span>
            </div>
          )
        ...
        {(!node.path || isExpanded(node.path)) && (
          <>
            {creating && creating.parent === (node.path || '') && (
              <div className="px-2 py-1" style={{ paddingLeft: 8 + childDepth * 12 }}>
                <input
                  ref={inputRef}
                  className="w-full text-sm"
                  style={{ color: '#dddddd', backgroundColor: '#1f1f22', border: '1px solid #3e3e42', padding: '2px 4px' }}
  ...
  return (
    <div className="flex-1 flex flex-col" style={{ minHeight: 0 }}>
      <div
        ...
        style={{ backgroundColor: 'var(--vscode-panel)', borderBottom: '1px solid var(--vscode-panel-border)', height: 'var(--header-height)', ...(dragOver === 'root' ? { boxShadow: 'inset 0 0 0 9999px var(--vscode-hover)' } : {}) }}
        onContextMenu={(e) => openContextMenu(e, '', 'root')}
        onDragOver={(e) => onDragOverTarget(e, 'root')}
        onDragLeave={onDragLeaveAny}
        onDrop={(e) => onDropTo(e, '')}
      >
        <div className="text-sm" style={{ color: 'var(--vscode-text)' }}>Explorer</div>
      ...
      <div
        ...
        style={{ padding: '6px 0', ...(dragOver === 'root' ? { boxShadow: 'inset 0 0 0 9999px var(--vscode-hover)' } : {}) }}
        onDragOver={(e) => onDragOverTarget(e, 'root')}
        onDragLeave={onDragLeaveAny}
        onDrop={(e) => onDropTo(e, '')}
        onContextMenu={(e) => openContextMenu(e, '', 'root')}
      ...
      {contextMenu && (
        <div
          ...
          style={{ top: contextMenu.y, left: contextMenu.x, backgroundColor: 'var(--vscode-panel)', border: '1px solid var(--vscode-panel-border)' }}
          onClick={(e) => e.stopPropagation()}
        >
          ...
          {contextMenu.type === 'folder' && (
            <>
              ...
              >New Folder</button>
              <div className="h-px" style={{ background: 'var(--vscode-panel-border)' }} />
              <button
                className="block w-full text-left px-3 py-2 text-sm"
                style={{ color: '#dddddd' }}
                onClick={() => {
              ...
              <button
                ...
                style={{ color: '#dd6666' }}
                onClick={() => {
                  if (!confirm(`Delete folder ${contextMenu.path} and its contents?`)) return;
