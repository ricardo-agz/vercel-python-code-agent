...
export const ThreePane: React.FC<ThreePaneProps> = ({
  ...
  const [activeMobilePane, setActiveMobilePane] = React.useState<'files' | 'code' | 'chat'>('code');
  ...
  if (isMobile) {
    return (
      <div className="h-screen w-screen flex flex-col" style={{ backgroundColor: 'var(--vscode-bg)' }}>
        {/* Mobile top bar */}
        <div
        ...
        >
          <button
            ...
            style={{
              ...
              boxShadow: activeMobilePane === 'files' ? 'inset 0 -2px 0 var(--vscode-accent)' : 'none',
            }}
          >
          ...
          <button
            ...
            style={{
              color: 'var(--vscode-text)',
              background: 'transparent',
              boxShadow: activeMobilePane === 'code' ? 'inset 0 -2px 0 var(--vscode-accent)' : 'none',
            }}
          >
          ...
          <button
            ...
            style={{
              color: 'var(--vscode-text)',
              background: 'transparent',
              boxShadow: activeMobilePane === 'chat' ? 'inset 0 -2px 0 var(--vscode-accent)' : 'none',
            }}
          >
        ...
        {/* Mobile content area */}
        <div className="flex-1 min-h-0 flex flex-col" style={{ paddingTop: 'var(--header-height)' }}>
          {activeMobilePane === 'files' && (
            <div className="flex-1 min-h-0 flex flex-col" style={{ backgroundColor: 'var(--vscode-sidebar)' }}>
              {left}
            </div>
          )}
          {activeMobilePane === 'code' && (
            <div className="flex-1 min-h-0 flex flex-col" style={{ minWidth: 0, minHeight: 0, paddingBottom: 'var(--statusbar-height)' }}>
              {center}
            </div>
          )}
  ...
  return (
    <div className="h-screen w-screen flex flex-col" style={{ backgroundColor: 'var(--vscode-bg)' }}>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col" style={{ minWidth: 0, minHeight: 0, paddingBottom: 'var(--statusbar-height)' }}>
          {center}
        </div>
