...
async def sandbox_stop(
    ...
    if not sid:
        ...
        try:
            sandbox = SANDBOX_CACHE.get(sid) or await Sandbox.get(sandbox_id=sid)
            await sandbox.stop()
            try:
                ...
                pass
            try:
                if sid in SANDBOX_CACHE:
                    await SANDBOX_CACHE[sid].client.aclose()
            except Exception:
                pass
            SANDBOX_CACHE.pop(sid, None)
            # Clear mappings
