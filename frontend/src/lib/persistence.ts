...
export function loadChatState(): PersistedChatsState | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as PersistedChatsState | Record<string, unknown>;
    const maybe = parsed as Record<string, unknown>;
    if (!parsed || typeof parsed !== 'object' || typeof maybe.threadsByProject !== 'object') {
...
export function getProjectChatThreads(projectId: string): PersistedChatThread[] {
...
function deriveTitleFromRuns(runs: Record<string, Run>, order: string[], createdAtISO: string): string {
  for (const id of order) {
    const r = runs[id];
    if (!r) continue;
    const firstUser = (r.actions || []).find((a: Action): a is UserMessageAction => a.kind === 'user_message');
...
function computeLastHumanAt(runs: Record<string, Run>, order: string[]): string | undefined {
  let latest: number = 0;
...
export function upsertCurrentChatThread(projectId: string, runs: Record<string, Run>, order: string[]): void {
  const base = loadChatState() || { threadsByProject: {}, version: 1 } as PersistedChatsState;
...
export function upsertThreadById(projectId: string, threadId: string, runs: Record<string, Run>, order: string[]): void {
  const base = loadChatState() || { threadsByProject: {}, version: 1 } as PersistedChatsState;
  const list = base.threadsByProject[projectId] || [];
  const nowISO = new Date().toISOString();
  ...
  if (idx === -1) {
    ...
    base.threadsByProject[projectId] = [created, ...list].slice(0, MAX_THREADS_PER_PROJECT);
    saveChatState({ ...base });
    return;
  }
...
export function setCurrentChatThread(projectId: string, threadId: string): PersistedChatThread | null {
  const base = loadChatState() || { threadsByProject: {}, version: 1 } as PersistedChatsState;
  const list = base.threadsByProject[projectId] || [];
  const idx = list.findIndex(t => t.id === threadId);
