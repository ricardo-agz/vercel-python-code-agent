  ...
  return (
    <div className="flex flex-col" style={{ height: `${height}px`, borderTop: '1px solid var(--vscode-panel-border)', backgroundColor: 'var(--vscode-bg)' }}>
      <div className="px-3 flex items-center justify-between" style={{ backgroundColor: 'var(--vscode-panel)', borderBottom: '1px solid var(--vscode-panel-border)', height: 'var(--header-height)' }}>
        <div className="text-xs font-medium" style={{ color: 'var(--vscode-text)' }}>{title}</div>
        <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--vscode-subtle)' }}>
          <button onClick={onClear} className="px-2 py-0.5 rounded-sm cursor-pointer" style={{ background: 'var(--vscode-surface)', color: 'var(--vscode-text)' }} disabled={!onClear}>Clear</button>
        </div>
      </div>
