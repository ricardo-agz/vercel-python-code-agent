...
import { Plus, Pencil, Trash2, Check, X, Download, MoreVertical, Copy } from 'lucide-react';
...
const ProjectTabs: React.FC<ProjectTabsProps> = ({ projects, activeProjectId, onSelect, onAdd, onRename, onDelete, onDownload, onClone }) => {
  ...
  return (
    ...
    >
          ...
          {projects.map((p) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <div className="flex items-center gap-1 px-2 relative" aria-label="Project actions" data-project-menu={p.id}>
                    {renamingId === p.id ? (
                      <>
                        <button
                          type="button"
      ...
      {openMenuId && menuRect && createPortal((() => {
        const MENU_WIDTH = 200;
        const left = Math.min(Math.max(8, menuRect.right - MENU_WIDTH), window.innerWidth - MENU_WIDTH - 8);
        const top = Math.min(menuRect.bottom + 6, window.innerHeight - 8 - 200);
        return (
          <div
          ...
          >
            <div
              ...
              style={{ left, top, background: 'var(--vscode-surface)', border: '1px solid var(--vscode-panel-border)', color: 'var(--vscode-text)', boxShadow: '0 8px 20px rgba(0,0,0,0.3)' }}
                ...
                return (
                  <>
                    <button
                      role="menuitem"
                      type="button"
                      onClick={() => { setRenamingId(p.id); setRenameValue(p.name); setOpenMenuId(null); }}
                    ...
                    <button
                      role="menuitem"
                      type="button"
                      onClick={() => { onClone(p.id); setOpenMenuId(null); }}
                      className="w-full flex items-center gap-2 px-3 py-1.5 text-left hover:bg-white/5"
                    ...
                    <button
                      role="menuitem"
                      type="button"
                      onClick={() => { onDelete(p.id); setOpenMenuId(null); }}
                      className="w-full flex items-center gap-2 px-3 py-1.5 text-left hover:bg-white/5"
