...
export default async function Page() {
  ...
  return (
    <main style={{ padding: 24, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <p>Backend says: <code>{apiRoot.message}</code></p>
      ...
      {items.length === 0 ? (
        ...
        <ul>
          {items.map((item: any) => (
            <li key={item.id}>
              {item.name} - ${""}{item.price.toFixed(2)} {item.description ? `(${item.description})` : ""}
            </li>
          ))}
        </ul>
      )}
      ...
      <p style={{ marginTop: 24, color: "#666" }}>
        Configure API URL with <code>NEXT_PUBLIC_API_URL</code> (defaults to http://localhost:8000)
      </p>
    </main>
  );
}
